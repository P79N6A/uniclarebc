...
docker exec -e "CORE_PEER_LOCALMSPID=LordshireMSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/Lordshire.example.com/users/Admin@Lordshire.example.com/msp" cli peer chaincode instantiate -o orderer.example.com:7050 -C mychannel -n college_chaincode -v 1.0 -c '{"Args":[]}' -P "OR ('LordshireMSP.member','StudentMSP.member','EmployerMSP.member','CollegeMSP.member')"



docker exec -e "CORE_PEER_LOCALMSPID=CollegeMSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/College.example.com/users/Admin@College.example.com/msp" cli peer chaincode invoke -o orderer.example.com:7050 -C mychannel -n college_chaincode -c '{"function":"enrollStudentToCollege","Args":["Rohit","M","22","9783365925","Test"]}'  
